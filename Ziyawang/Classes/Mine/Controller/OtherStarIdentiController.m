...
- (void)setViews
{
    ...
    [self setButtonWithButton:self.renzhengButton];
    if ([self.role isEqualToString:@"1"]) {
        [self.renzhengButton setHidden:YES];
        self.label1.text = @"可选择完成以下认证点亮星级";
    }
    else
    {
        self.label1.text = @"您需要先通过服务方认证才可进行星级认证";
        [self.openButton1 setHidden:YES];
        [self.openButton2 setHidden:YES];
