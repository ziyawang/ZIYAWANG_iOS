...
- (void)initCityPickerView{
    ...
    UIView *pickerBackView = [[UIView alloc]initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height - 300, [UIScreen mainScreen].bounds.size.width, 300)];
    
    pickerBackView.backgroundColor = [UIColor whiteColor];
    
    _cityPicker = [[UIPickerView alloc]initWithFrame:CGRectMake(0, 50,pickerBackView.bounds.size.width,150)];
    
    //一个选择器
//    _cityPicker=[[UIPickerView alloc] initWithFrame:CGRectMake(0, KScreenHeight-300, KScreenWidth, 300)];
...
//    [self addSubview:_cityPicker];
    ...
    [btnCancel setFrame:CGRectMake(10, 10, 40, 30)];
    
//    UIButton *btnCancel=[[UIButton alloc] initWithFrame:CGRectMake(15, 5, 40, 30)];
...
//    [btnCancel setTitleColor:[UIColor lightGrayColor] forState:UIControlStateHighlighted];
    ...
    [btnSure setFrame:CGRectMake(KScreenWidth-50, 10, 40, 30)];
    
//    UIButton *btnSure=[[UIButton alloc] initWithFrame:CGRectMake(KScreenWidth-55, 5, 40, 30)];
...
- (void)getCityData{
    ...
    _arrProvince = [[NSArray alloc]initWithContentsOfFile:plistPath];
    
    //    _arrProvince=[addressDic objectForKey:@"address"];
    _arrCity=[[_arrProvince objectAtIndex:0] objectForKey:@"cities"];
    
    //    _arrDistrict=[[_arrCity objectAtIndex:0] objectForKey:@"sub"];
    _dicSelectCityAndCityCode=[[NSMutableDictionary alloc] init];
    
    [_dicSelectCityAndCityCode setObject:[[_arrProvince objectAtIndex:0] objectForKey:@"state"] forKey:@"Province"];
    
    [_dicSelectCityAndCityCode setObject:[[_arrCity objectAtIndex:0] objectForKey:@"city"] forKey:@"City"];
...
//        return [[_arrProvince objectAtIndex: row] objectForKey:@"state"];
//    }else if (component == CITY_COMPONENT) {
//        return [[_arrCity objectAtIndex: row] objectForKey:@"city"];
//    }else {
...
{
    if (component == PROVINCE_COMPONENT) {
        _arrCity=[[_arrProvince objectAtIndex:row] objectForKey:@"cities"];
...
//        }
        ...
        [_dicSelectCityAndCityCode setObject:[[_arrProvince objectAtIndex:row] objectForKey:@"state"] forKey:@"Province"];
...
//        }
        ...
        [_dicSelectCityAndCityCode setObject:[[_arrCity objectAtIndex:row] objectForKey:@"city"] forKey:@"City"];
//        [_dicSelectCityAndCityCode setObject:[_arrDistrict objectAtIndex:0] forKey:@"District"];
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view
{
    ...
    if (component == PROVINCE_COMPONENT) {
        ...
        myView.text = [[_arrProvince objectAtIndex:row] objectForKey:@"state"];
        myView.font = [UIFont systemFontOfSize:17];
        myView.backgroundColor = [UIColor clearColor];
    }else if (component == CITY_COMPONENT) {
        myView = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 0.0, KScreenWidth/3.0, 30)];
        myView.textAlignment = 1;
        myView.text = [[_arrCity objectAtIndex:row] objectForKey:@"city"];
        myView.font = [UIFont systemFontOfSize:17];
        myView.backgroundColor = [UIColor clearColor];
    }else {
        myView = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 0.0, KScreenWidth/3.0, 30)];
